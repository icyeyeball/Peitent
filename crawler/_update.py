...
cmd_users = "SELECT indexNo, applNo FROM tmarkTable WHERE imageData1 = ./picBase/000000000-1.png"
cursor.execute(cmd_users)
tmark_list = cursor.fetchall()

for index in range(len(tmark_list[index][0])):
    ...
    url = 'https://tiponet.tipo.gov.tw/OpenDataApi/OpenData/API/TmarkRights?format=xml&top=400&skip='+str(tmark_list[index][0])+'&orderby=appl-no&tk=ywgvRgZ1'
