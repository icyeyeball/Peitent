...
outputPath='./7163-2/' #path of database
tmpfiles = os.listdir(outputPath)
for f in tmpfiles:
    os.remove('./7163-2/'+str(f))

sampleImage = cv2.imread(samplePath,0)

sampleImage1 = imutils.resize(sampleImage, width = 300)
sampleImage1 = cv2.GaussianBlur(sampleImage1, (3, 3), 0)
...
for t in tmark_l:
    ...
    try:
        ...
        queryImage1 = cv2.GaussianBlur(queryImage, (3, 3), 0)
...
for k in range(0,len(result)):
    outpath = "./7163-2/" + str(k+1) + "-" +"("+str(round(result[k]["ratio"],3)) + "-" + str(result[k]["applno"]) +").jpg"
    #print ("===========================")
    #print (outpath)
