...
for index in range(39370 ,3000000):
        ...
        #time.sleep(60)
    url = 'https://tiponet.tipo.gov.tw/OpenDataApi/OpenData/API/TmarkRights?format=xml&top=10&skip=39377&orderby=appl-no&tk=ywgvRgZ1'
    ...
    if isOK == ['ok']:
        ...
        for i in range(0, len(tmarkColorDesc)):
            if len(tmarkColorDesc[i]) >0 and None not in tmarkColorDesc:
        ...
        if len(tmarkDraftC) > 0:
           for i in range(0, len(tmarkDraftC)):
               if tmarkDraftC[i] != [] and None not in tmarkDraftC:
        ...
        if len(tmarkDraftE) > 0:
            for i in range(0, len(tmarkDraftE)):
                if tmarkDraftE[i] != []  and None not in tmarkDraftE:
        ...
        if len(tmarkDraftJ) > 0:
            for i in range(0, len(tmarkDraftJ)):
                if tmarkDraftJ[i] != [] and None not in tmarkDraftJ:
        ...
        for i in range(0, len(wordDescription)):
            if wordDescription[i] != [] and None not in wordDescription:
        ...
        for t in tree.xpath("/API/tmarkrights/tmarkcontent/goodsclasses/goodsclass"):
            if t.attrib.has_key('sequence'):
                string0= str(cop.sub('', str(t.attrib.has_key)))
                goodsNo.append(string0)
        print(goodsNo)

        if len(goodsNo) > 0:
            for i in range(0, len(goodsNo)):
                if i == len(goodsNo)-1 and goodsNo[i] =='1':
                    string1 = str(goodsClassCode[i])
                    newGoodsClassCode.append(string1)
                elif goodsNo[i] == '1' and goodsNo[i+1] == '1':
                    string1 = str(goodsClassCode[i])
                    newGoodsClassCode.append(string1)
                elif i < len(goodsNo)-1 and goodsNo[i] == '1':
                    string1 = ""
                    string1 = string1 + str(goodsClassCode[i])
                elif i < len(goodsNo)-1 and goodsNo[i] != '1' and goodsNo[i+1] == '1':
                    string1 = string1 + " " + str(goodsClassCode[i])
                    newGoodsClassCode.append(string1)
                elif i < len(goodsNo)-1 and goodsNo[i] != '1' and goodsNo[i+1] != '1':
                    string1 = string1 + " " + str(goodsClassCode[i])
        ...
        if len(goodsName) > 0:
            for i in range(0, len(goodsName)):
                if len(goodsName[i]) >0 and None not in goodsName:
        ...
        for i in range(0, len(processorName)):
            if processorName[i] != [] and None not in processorName:
        ...
        string1 = ""
        string2 = ""
        for t in tree.xpath("/API/tmarkrights/tmarkcontent/parties/agents/agent"):
            if t.attrib.has_key('sequence'):
                string0= cop.sub('', str(t.attrib.has_key))
                if len(string0) != 0:
                    agentNo.append(string0)
                else:
                    agentNo.append("0")
        print(agentNo)
        
        if len(agentNo) > 0:
            for i in range(0, len(agentNo)):
                if i == len(agentNo)-1 and agentNo[i] =='1':
                    string1 = str(agentChineseName[i])
                    string2 = str(agentAddress[i])
                    newAgentChineseName.append(string1)
                    newAgentAddress.append(string2)
                elif agentNo[i] == '1' and agentNo[i+1] == '1':
                    string1 = str(agentChineseName[i])
                    string2 = str(agentAddress[i])
                    newAgentChineseName.append(string1)
                    newAgentAddress.append(string2)
                elif i < len(agentNo)-1 and agentNo[i] == '1':
                    string1 = ""
                    string2 = ""
                    string1 = string1 + str(agentChineseName[i])
                    string2 = string2 + str(agentAddress[i])
                elif i < len(agentNo)-1 and agentNo[i] != '1' and agentNo[i+1] == '1':
                    string1 = string1 + " " + str(agentChineseName[i])
                    string2 = string2 + " " + str(agentAddress[i])
                    newAgentChineseName.append(string1)
                    newAgentAddress.append(string2)
        ...
        print(newAgentChineseName)
        print(newAgentAddress)
