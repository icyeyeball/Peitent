...
sampleImage1 = cv2.GaussianBlur(sampleImage1, (1, 1), 0)
kp1_1, des1_1 = sift.detectAndCompute(sampleImage1, None) #detect the features of sample

index = 0
localtime_middle = time.asctime( time.localtime(time.time()))
...
for t in tmark_l:
    ...
    try:
        ...
        kp2_1, des2_1 = sift.detectAndCompute(queryImage, None) #detect the features of img in database

        try:
            matches2=flann.knnMatch(des2_1,des1_1,k=2) #matched features, assign k=2 to return 2 matched features.
        except:
            continue
        else:
            ...
            if len(matches2) != 0:
                ...
                matchRatio2_2=0.
        try:
            matches2=flann.knnMatch(des1_1,des2_1,k=2) #matched features, assign k=2 to return 2 matched features.
        except:
            continue
        else:
            (matchNum1,matchesMask1)=getMatchNum(matches2,0.9) #set ratio = 0.9 to calculate the matching level
            if len(matches2) != 0:
                matchRatio1_2=matchNum2*100/len(matches2)
            else:
                matchRatio1_2=0.           
            matchRatio = (matchRatio1_2 + matchRatio2_2)/2.       
            ...
            subtotal = {"applno":t["applno"],"ratio":matchRatio, "picture":queryImage}
...
print("開始時間: "+ localtime_init)   
print("資料庫撈取完並處理後時間: "+ localtime_middle)  
